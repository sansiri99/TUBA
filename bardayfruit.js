...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const kitchenType = 'บาร์รายวันผลไม้สด';
  ...
  handleFinalSubmit('finalSubmitButton', 'stockForm', filteredItems, async function(items, formId) {
    ...
    reader.onload = async function(event) {
      ...
      const fileName = `${getFormattedDateTime()}_นับสต๊อก_บาร์_รายวัน_ผลไม้สด.csv`;
      ...
      uploadForm.append('filename', fileName);
        ...
        try {
          ...
          hideLoadingSpinner();
          if (result.url) {
            sendLineNotify('bardayfruit', result.url, employeeName);  // Pass the employee name to LINE Notify function
            window.location.href = `complete.html?fileUrl=${encodeURIComponent(result.url)}&formType=bardayfruit&employeeName=${encodeURIComponent(employeeName)}`;
          } else {
