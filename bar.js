...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Filter stock items for the bar
  const filteredItems = stockItems.filter(item => item.kitchen === 'บาร์');
    ...
    reader.onload = function(event) {
      ...
      .then(result => {
        if (result.url) {
          ...
          window.location.href = `complete.html?fileUrl=${encodeURIComponent(result.url)}&formType=bar`;
        } else {
