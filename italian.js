...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const isItalianKitchen = pageTitle.includes('ครัวอิตาเลี่ยน');
  const kitchenType = isItalianKitchen ? 'ครัวอิตาเลี่ยน' : 'ครัวไทย';

  // Filter stock items for the selected kitchen
  const filteredItems = stockItems.filter(item => item.kitchen === (isItalianKitchen ? 'อิตาเลี่ยน' : 'ไทย'));
  ...
  navigateButton.addEventListener('click', function() {
    if (confirm(`คุณแน่ใจหรือว่าต้องการไปยังหน้าการจัดการสต็อก - ครัว${isItalianKitchen ? 'ไทย' : 'อิตาเลี่ยน'}?`)) {
      location.href = isItalianKitchen ? 'thai.html' : 'italian.html';
    }
  });
});
